...
class RekapSekolahController extends Controller
{
    ...
    public function getRekapSekolahRingkasanSp(Request $request){
        ...
        if(!Cache::get('getRekapSekolahRingkasanSp_'.$request->input('id_level_wilayah').$request->input('kode_wilayah').$request->input('sekolah_id').$request->input('keyword').date('Y-m-d'))){
            ...
            WHERE
            ".(!$request->input('sekolah_id') && !$request->input('keyword') ? "semester_id = '{$request->input('semester_id')}' AND tahun_ajaran_id = '{$request->input('tahun_ajaran_id')}' ".($request->input('kode_wilayah') ? "AND {$params_wilayah} = '{$request->input('kode_wilayah')}'" : "")." " : "")."
                ".($request->input('sekolah_id') ? "rekap_sekolah.sekolah_id = '{$request->input('sekolah_id')}'" : "")."
                ".($request->input('keyword') ? "rekap_sekolah.nama like '%{$request->input('keyword')}%' OR rekap_sekolah.npsn like '%{$request->input('keyword')}%'" : "")."
                AND soft_delete = 0
                {$param_bentuk}
                {$param_status}
                {$param_keyword}";
            ...
            $sql = "SELECT
                ROW_NUMBER() OVER (ORDER BY rekap_sekolah.nama) as 'no',
                ...
                ISNULL((case when rekap_sekolah.status_sekolah = 1 or rekap_sekolah.status_sekolah = 2 then rekap_sekolah.pd else 0 end ), 0) as pd,
                ISNULL((case when rekap_sekolah.status_sekolah = 1 or rekap_sekolah.status_sekolah = 2 then rekap_sekolah.guru else 0 end ), 0) as guru,
                ISNULL((case when rekap_sekolah.status_sekolah = 1 or rekap_sekolah.status_sekolah = 2 then rekap_sekolah.pegawai else 0 end ), 0) as pegawai,
                ISNULL((case when rekap_sekolah.status_sekolah = 1 or rekap_sekolah.status_sekolah = 2 then rekap_sekolah.rombel else 0 end ), 0) as rombel,
                rekap_sekolah.tanggal as tanggal_rekap_terakhir
            FROM
                rekap_sekolah WITH(NOLOCK)
            LEFT JOIN datamart.dbo.sekolah dm_sekolah on dm_sekolah.sekolah_id = rekap_sekolah.sekolah_id and dm_sekolah.semester = '20202'
            WHERE
                ".(!$request->input('sekolah_id') && !$request->input('keyword') ? "semester_id = '{$request->input('semester_id')}' AND tahun_ajaran_id = '{$request->input('tahun_ajaran_id')}' ".($request->input('kode_wilayah') ? "AND {$params_wilayah} = '{$request->input('kode_wilayah')}'" : "")." " : "")."
                ".($request->input('sekolah_id') ? "rekap_sekolah.sekolah_id = '{$request->input('sekolah_id')}'" : "")."
                ".($request->input('keyword') ? "rekap_sekolah.nama like '%{$request->input('keyword')}%' OR rekap_sekolah.npsn like '%{$request->input('keyword')}%'" : "")."
            ...
            $sql .= " ORDER BY ".($request->sekolah_id || $request->keyword ? "rekap_sekolah.semester_id desc" : "rekap_sekolah.nama asc")." OFFSET ".($request->input('start')?$request->input('start'):0)." ROWS FETCH NEXT ".($request->sekolah_id || $request->keyword ? 1 : ($request->input('limit')?$request->input('limit'):20))." ROWS ONLY";
            ...
            Cache::put('getRekapSekolahRingkasanSp_'.$request->input('id_level_wilayah').$request->input('kode_wilayah').$request->input('sekolah_id').$request->input('keyword').date('Y-m-d'), $return);

            return $return;
        }else{
            return Cache::get('getRekapSekolahRingkasanSp_'.$request->input('id_level_wilayah').$request->input('kode_wilayah').$request->input('sekolah_id').$request->input('keyword').date('Y-m-d'));
        }
    }
