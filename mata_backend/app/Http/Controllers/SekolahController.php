...
class SekolahController extends Controller
{
    ...
    public function getSekolahSekitar(Request $request){
        $sekolah_id = $request->sekolah_id ? $request->sekolah_id : null;

        $query = "SELECT
            sekolah.sekolah_id,
            smp_sekitar.nama,
            sekolah.lintang AS lintang_1,
            sekolah.bujur AS bujur_1,
            smp_sekitar.lintang AS lintang_2,
            smp_sekitar.bujur AS bujur_2,
            dbo.Distance ( smp_sekitar.lintang, smp_sekitar.bujur, sekolah.lintang, sekolah.bujur ) 
        FROM
            sekolah
            LEFT JOIN ( SELECT nama, sekolah_id, lintang, bujur FROM sekolah WHERE bentuk_pendidikan_id = '6' ) smp_sekitar ON dbo.Distance ( smp_sekitar.lintang, smp_sekitar.bujur, sekolah.lintang, sekolah.bujur ) <= 10 and dbo.Distance ( smp_sekitar.lintang, smp_sekitar.bujur, sekolah.lintang, sekolah.bujur ) > 0
        WHERE
            sekolah.sekolah_id = '".$sekolah_id."'";

        $fetch = DB::connection('sqlsrv_dm')->select($query);
